...
export const DeviceView: React.FC<{ isDemo: boolean }> = ({isDemo}) => {
    ...
    // const { data: devices, refetch: refetchQuery } = api[isDemo ? "user_client" : "user_client"].getAllDevices.useQuery(
    //     { userId: userId },
    //     { enabled: sessionData?.user !== undefined },
    // );

    const { data: devices, refetch: refetchQuery } = isDemo ? api.demo_client.getAllDevices.useQuery(
        { userId: userId },
    ...
    return (
        <>
            ...
            {popupVisible && <EditDevicePopup hidden={!popupVisible} setHidden={() => {setPopupVisible(false); refetchTimer();}} deviceId={editDeviceId} deviceName={editDeviceName} isDemo={isDemo} />}
            {newDevicePopupVisible && <NewDevicePopup hidden={!newDevicePopupVisible} setHidden={() => {setNewDevicePopupVisible(false); refetchTimer();}} isDemo={isDemo} />}
        </>
    );
}